...
async function processPolitician(profile, idCache, dataCache) {
  ...
  const candidate = await fecCandidateSearch(name, idCache)
  if (!candidate) {
...
async function main() {
  ...
  let targetPoliticians = politicians
  let targetDonors = donors

  // Apply profile filter
  if (PROFILE_FILTER) {
    const filter = PROFILE_FILTER.toLowerCase()
    targetPoliticians = politicians.filter(p =>
      extractName(p.title).toLowerCase().includes(filter))
    targetDonors = donors.filter(d =>
  ...
  for (const profile of targetPoliticians) {
    ...
    if (WRITE && result.status === "success" && result.totals) {
      ...
      if (VERBOSE) console.log(`    Updated frontmatter: ${path.relative(CONTENT_DIR, profile.filePath)}`)
    }
  }

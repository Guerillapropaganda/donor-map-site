...
1. **Run FOUR single-candidate Gemini prompts** for the non-Bass LA Mayor field (Pratt / Raman / Huang gap-fill / Miller). The original consolidated 4-in-1 prompt was too much context for Gemini to keep straight (David flagged it returned weak data). Rewritten as four separate single-candidate prompts at end of this session — each is its own copy-paste unit with identity-verification first, single-target focus, and Tier-1 source mandate. Adam Miller prompt specifically leads with disambiguation (LA has multiple Adam Millers in public-figure space). Huang prompt is gap-fill-only since rounds 1 and 2 already covered policy / endorsements / aggregate fundraising — round 3 only needs granular donor list + Form 700 + Form 410.

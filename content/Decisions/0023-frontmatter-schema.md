...
Coverage in current corpus: these five are ≥99% populated on
**editorial-shape** content types (politician, donor, corporation,
story, media-profile, pac, think-tank, lobbying-firm, sub-note,
state/local-politician, policy).

**Exemptions amended 2026-04-24.** Two content types are log-shape,
not editorial-shape, and are exempt from some universals:

- **event** — exempt from `last-updated`, `source-tier`,
  `content-readiness`. Events use `date` as their authoritative
  timestamp and `status` as their lifecycle marker. No editorial
  review applies; they're factual log entries with `source` +
  `source-url` fields already covering provenance.
- **story-seed** — exempt from `source-tier`, `content-readiness`.
  Seeds are auto-generated by scripts and use their own
  `confidence` + `status` + `auto-generated` fields. Editorial
  promotion happens when a seed is upgraded to a full `story`
  profile, which then carries its own universal fields.

The exemptions are encoded in `scripts/lib/frontmatter-schema.cjs`
as `exempt_universal: [...]` per-type overrides. The validator
honors them.

Without these exemptions, the first validator run (2026-04-24)
reported 1937 universal-missing findings, 696 of which were events
missing `content-readiness` — a field that is meaningless for an
event. The amendment reconciles the schema with reality. Real
universal-gap findings after amendment: 25 `source-tier` cases on
editorial profiles (small, manual-fix targets).
...
- **amended 2026-04-24** — removed `last-enriched`. Sub-notes are
  editorial child documents, not pipeline-enriched entities. 450 of
  462 had no `last-enriched` field; the requirement was unfounded.
...
- **amended 2026-04-24** — removed `last-enriched`. Stories are
  narrative content written against existing data; they're not
  pipeline-enriched. 105 of 105 stories had no `last-enriched`
  field; the requirement was unfounded.

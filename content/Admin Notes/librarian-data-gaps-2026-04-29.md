...
4. ~~Investigate WHO deleted `fec-pas2.jsonl` originally~~ **Investigated 2026-04-29.** No script in the repo unlinks the file. The likely cause is a manual `rm` or `git clean -fdx` (which removes untracked-and-gitignored files) at some point after the April 28 morning regen+gitignore commit (`de83a77f0`). The deleter remains unknown but the structural fix is more useful than the forensics: added `data/derived/fec-pas2.jsonl` to `scripts/ensure-derived-artifacts.cjs` ARTIFACTS list, so the file auto-regenerates via the post-checkout / post-merge git hooks whenever it's missing. Whoever deletes it next time, it comes back on the next branch switch.

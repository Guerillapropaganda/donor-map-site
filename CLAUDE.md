...
- **ADR-0030** — Code-Audit External Access Carve-out (accepted 2026-04-30). Carve-out from Rule 13: Code Claude may fetch government primary sources (Phase 1: Cal-Access; Phase 2 by amendment: FEC, IRS, SEC, FPPC, Congress) for the narrow purpose of verifying its own pipeline output against the source the pipeline claims to read from. Every fetch logs to `data/code-audit-fetches.jsonl`. Discrepancies file as Tier 2 corrections via the editorial-decision-pipeline (David approves) — never auto-applied to profile content. New sentinel (`code-audit-fetch-sentinel`) mechanically blocks fetched URLs from leaking into profile bodies. News, aggregators, social media explicitly out of scope. Defamation surface unchanged. Implementation phased: helper + sentinel + harness check + first audit script (`audit-cal-access-overrides.cjs`) ship together, ~$15-25 Opus.

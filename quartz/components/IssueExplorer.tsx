  ...
  function showIssue(issueName) {
    ...
    if (issue.donors.length > 0) {
      ...
      for (var d = 0; d < issue.donors.length; d++) {
        ...
        var status = (donor.readiness === 'verified' || donor.readiness === 'ready' || donor.readiness === 'publication-ready') ? 'SOURCED' : 'DRAFT';
        var statusClass = status === 'SOURCED' ? 'ie-sourced' : 'ie-draft';

...
const SECURITY_SEED = {
  items: [
    // --- Infrastructure ---
    { id: "cf-proxy", name: "Cloudflare DNS proxy", description: "Route all traffic through Cloudflare for DDoS protection, CDN, and origin IP hiding", status: "done", priority: "critical", category: "infrastructure", notes: "Activated 2026-04-15. Nameservers: liv.ns.cloudflare.com + tadeo.ns.cloudflare.com. SSL: Full mode. HSTS enabled (6mo). Min TLS 1.2. Always HTTPS on.", updatedAt: now() },
    { id: "sec-headers", name: "Security headers (X-Frame, X-Content-Type, Referrer, Permissions)", description: "Transform Rule 'Security Headers' adds X-Frame-Options DENY, X-Content-Type-Options nosniff, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy camera/mic/geo denied", status: "done", priority: "critical", category: "application", notes: "Deployed 2026-04-15 via Cloudflare Transform Rules. 1/10 rules used.", updatedAt: now() },
    { id: "branch-protect", name: "Branch protection on v4", description: "Require status checks (regression, data integrity, ops build), block force push + branch deletion. No PR review required (solo dev).", status: "done", priority: "high", category: "infrastructure", notes: "Enabled 2026-04-15 via GitHub API. Admin bypass on for direct pushes.", updatedAt: now() },
    { id: "pin-actions", name: "Pin GitHub Action versions to SHAs", description: "All 7 project-owned workflows pinned to immutable commit SHAs. Quartz upstream .yaml files left unpinned.", status: "done", priority: "medium", category: "infrastructure", notes: "Shipped 2026-04-15. checkout, setup-node, cache, upload-pages-artifact, deploy-pages, github-script all pinned.", updatedAt: now() },
    ...
    // --- Application ---
    { id: "turnstile", name: "Cloudflare Turnstile on tip form", description: "Invisible CAPTCHA integrated in TipForm.tsx. Needs site key from CF dashboard.", status: "in-progress", priority: "high", category: "application", notes: "Code shipped 2026-04-15. David: CF dashboard > Turnstile > Add Widget > Invisible mode > copy site key > replace TURNSTILE_SITE_KEY_PLACEHOLDER in quartz/components/TipForm.tsx.", updatedAt: now() },
    { id: "worker-ratelimit", name: "Edge rate limiting (Cloudflare Worker KV)", description: "60 req/min per IP. Worker code ready at scripts/cloudflare/rate-limit-worker.js.", status: "in-progress", priority: "high", category: "application", notes: "Code shipped 2026-04-15. David: CF dashboard > Workers > Create > paste code > bind KV namespace 'rate-limit-store' > add route thedonormap.org/*.", updatedAt: now() },
    ...
    { id: "analytics", name: "GoatCounter analytics", description: "Privacy-respecting, cookie-free traffic analytics. Wired into quartz.config.ts.", status: "in-progress", priority: "medium", category: "monitoring", notes: "Config shipped 2026-04-15 (websiteId: donormap). David: sign up at goatcounter.com (free), create site with code 'donormap', then stats appear at donormap.goatcounter.com.", updatedAt: now() },
    // --- Personal ---
    ...
    { id: "2fa", name: "2FA on all accounts", description: "Enable 2FA on GitHub, Cloudflare, Namecheap, ProtonMail. Prefer authenticator app or hardware key over SMS.", status: "done", priority: "critical", category: "personal", notes: "All accounts secured 2026-04-15. GitHub, Namecheap, ProtonMail.", updatedAt: now() },
    // --- Legal ---

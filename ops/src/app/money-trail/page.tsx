...
interface MoneyEdge extends SimulationLinkDatum<MoneyNode> {
  confidence: number; amount: number; edgeType: string
}
...
function getNodeSize(n: MoneyNode): number {
  // Size by dollar amount when available, fallback to degree
  if (n.totalAmount > 0) {
    const logAmt = Math.log10(n.totalAmount + 1)
    return 4 + Math.min(logAmt * 2, 22)
  }
  return 4 + Math.min(Math.sqrt(n.degree) * 2.5, 18)
}
...
export default function MoneyTrailPage() {
  ...
  const buildGraph = useCallback(() => {
    ...
    const sim = forceSimulation<MoneyNode>(simNodes)
      ...
      .force("collide", forceCollide<MoneyNode>(n => getNodeSize(n) + 2).iterations(2))
    ...
    const edgeEls = edgeGroup.selectAll("line")
      .data(simEdges)
      .join("line")
      .attr("stroke", (d: any) => EDGE_COLORS[d.edgeType] || "#f59e0b")
      .attr("stroke-width", (d: any) => 0.5 + Math.min((d.amount || 0) / 1e7, 2))
    ...
    nodeEls
      .on("mouseenter", function (event, d) {
        select(this).select("circle").attr("r", getNodeSize(d) + 3)
        ...
        setHovered(d.name + (d.totalAmount > 0 ? ` — ${formatAmt(d.totalAmount)}` : ""))
      })
      .on("mouseleave", function (event, d) {
        select(this).select("circle").attr("r", getNodeSize(d))
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 py-1.5 border-b border-[var(--color-border)] flex-wrap">
        ...
        <span className="text-[7px] text-[var(--color-text-dim)]">Arrows show money flow | Size = dollar amount | <span style={{color:"#22c55e"}}>Green</span> = donations | <span style={{color:"#3b82f6"}}>Blue</span> = contracts | <span style={{color:"#ef4444"}}>Red</span> = opposition | Scroll zoom | Drag nodes</span>
      </div>
